...
if ($_GET['action'] == "get_list_shopify_shop") {
    $shop_list = array();
    $shop_list_query = $wpdb->get_result("SELECT `shop`,`id`,`active` FROM `wp_users_shopify` WHERE `users_id` = $user_id");
...
if ($_GET['action'] == "get_list_bigcommerce_shop") {
    $shop_list = array();
    $shop_list_query = $wpdb->get_result("select `shop`,`id`,`active` from `wp_users_bigcommerce` where `users_id` = $user_id");
    $numshops_bigc = $wpdb->num_rows($shop_list_query);
...
if ($_GET['action'] == "get_list_opencart_shop") {
    $shop_list = array();
    $shop_list_query = $wpdb->get_result("select `domain`,`id`,`active` from `wp_users_opencart` where `users_id` = $user_id");

    if ($wpdb->num_rows($shop_list_query) == 0)
