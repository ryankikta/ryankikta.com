...
function buildBigcommerceVariants($variants)
{
    ...
    if ($_SERVER['REMOTE_ADDR'] == "59.162.181.90") {
        ...
        foreach ($variants as $key => $variant) {
            ...
            $bigcommerce_variants[] = array("color_name" => $variant['color_name'], "size_name" => $variant['size_name'], "sku" => $variant['sku'], "price" => $variant['price'], "position" => $variant['position'], "color_code" => $variant['color_code'], "color_id" => $variant['color_id'], "size_id" => $variant['size_id'], "image_id" => $variant['image_id'], "image_url" => $variant['image_url']);
...
function updateBigcommerceVariants($POST, $big_id, $products_id, $auth, $variants, $basicprice, $userid, $shop_id)
{
    ...
    foreach ($variants as $key => $variant) {
        ...
        $image_url = (isset($variant['image_url'])) ? $variant['image_url'] : "";
        ...
        //mail("team@ryankikta.com","createProductSku response",var_export($skucreated,true));
        if ($POST['pagetype'] == 1) {
            $errors = Bigcommerce::getLastError();
            $errors = processBigcommerceErrors($errors);
        }
        ...
        if ($variant_sku != "") {
            ...
            if ($product) {
                ...
                if ($pa_product_id > 0) {
                    ...
                    $variant_sku = esc_sql($variant_sku);
                    if ($variant_id == 0) {
...
function regenerate_big_order($order_id, $shop_id, $type = 0)
{
    ...
    if (!empty($auth)) {
        ...
        curl_setopt($ch, CURLOPT_URL, 'https://ryankikta.com/bigcommerce-orders/?shop_id=' . $shop_id);
...
function get_listShipments_order_bigcommerce($order_id)
{
    global $wpdb;
    $print_orderid = $wpdb->get_row("select `orderid`,`shop_id` from `wp_rmproductmanagement_orders` where `order_id` = " . $order_id, ARRAY_A);
    $shop_id = $print_orderid['shop_id'];
    ...
    $auth = getBigcommerceShopbyId($shop_id);
    bigcommerce_connect($auth);
    $result = callBigcommerceApi("getOrderShipments", $order_Bigid[1]);
    return $result;
...
function callBigcommerceApi($method, $args = array(), $lenght = 1)
{
    if ($lenght == 2) {
